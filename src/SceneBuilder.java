...
public class SceneBuilder extends Application {
  ...
  public void start(Stage primaryStage) throws Exception {
    //FXMLLoader loader = new FXMLLoader(new File("res/style/DefaultDictionary.fxml").toURI().toURL());
    //Parent root = loader.load();
    Parent root = FXMLLoader.load(getClass().getResource("res/style/DefaultDictionary.fxml"));
    primaryStage.setTitle("Dictionary");
    Scene scene = new Scene(root, 890, 532);
      ...
      alert.setHeaderText("This word is not existed");
        if (fuzzy == null) {
            ...
            alert.setContentText(
                "Did you mean " + fuzzy + ": " + dictapp.getDdict().lookup(fuzzy) + "?");
        }
      alert.show();
  ...
  private void addNewWordtoDefaultDictionary(Word word) {
    ...
    if (!dictapp.getDdict().getDictionary()
        .addNewWord(word.getWordTarget(), word.getWordExplain())) {
      Alert alert = new Alert(Alert.AlertType.INFORMATION);
      alert.setTitle("Error...");
  ...
  public void dd_onClickListenIcon() throws Exception {
    VoiceProvider tts = new VoiceProvider("cf99ca3636f34559bc565c329cd6801c");
    System.out.println("Word target on dd_onClickListenIcon(): " + dd_wordTarget);
    VoiceParameters params = new VoiceParameters(dd_wordTarget.toLowerCase(Locale.ROOT),
    ...
    FileOutputStream fos = new FileOutputStream("res\\voice.wav");
    fos.write(voice, 0, voice.length);
    fos.flush();
    fos.close();

    String path = "res\\voice.wav";
  ...
  public void md_onClickListenIcon() throws Exception {
    ...
    VoiceParameters params = new VoiceParameters(md_wordTarget.toLowerCase(Locale.ROOT),
        Languages.English_UnitedStates);
    params.setCodec(AudioCodec.WAV);
    params.setFormat(AudioFormat.Format_44KHZ.AF_44khz_16bit_stereo);
    params.setBase64(false);
