...
public class RBTree_additive {
  ...
  ///情况二///cur///情况一/
  private RBTNode<additiveMyCpr1> downwardNeighbour(RBTNode<additiveMyCpr1> du,
      additiveMyCpr1 val) {
    int cmp;
  ...
  public void insert(String key, int... val) {
    ...
    while (x != null) {
      ...
      cmp = key.toLowerCase().replace(" ", emptyStr).replace("'", emptyStr).replace(":", emptyStr)
          .replace(".", emptyStr).replace("-", emptyStr).replace(",", emptyStr).compareTo(
              x.key.key.toLowerCase().replace(" ", emptyStr).replace("'", emptyStr)
                  .replace(":", emptyStr).replace(".", emptyStr).replace("-", emptyStr)
                  .replace(",", emptyStr));
      ...
      if (cmp < 0) {
        ...
        for (int i : val) {
          ((ArrayList<Integer>) x.key.value).add(i);
        }
        return;//here
      }
    }
    ...
    for (int i : val) {
      ((ArrayList<Integer>) node_key.value).add(i);//here
    }
    ...
    if (tree != null) {
      ...
      } else                // tree是分支节点
      {
        System.out.printf("【%s(%s)】 is 【%s】's %6s child\n", tree.key.toString(),
            isRed(tree) ? "R" : "B", key.toString(), direction == 1 ? "right" : "left");
      }
