...
public class MdTest {
  ...
  public static void main(String[] args) throws IOException, DataFormatException {
    ...
    if (false) {  /* false  true  */
      ...
      CMN.show(
          "模糊搜索完成！ 耗时" + (System.currentTimeMillis() - stst) + "ms,共搜索到 " + resCount
              + "个词条！");
    }
    ...
    if (true) {  /* false  true  */
      ...
      CMN.show("多线程全文搜索测试 time used： " + (System.currentTimeMillis() - stst)
          + "ms 共搜索到res count: " + gross.size());
  ...
  public static void harvestfuzzySearch(boolean isCombinedSearching, int adapter_idx,
      ArrayList<mdict> mdicts) {
    ...
    for (mdict mdI : mdicts) {
      ...
      if (fuzzy_task_count <= 0) {
        ...
        CMN.show("模糊搜索完成! 耗时" + (System.currentTimeMillis() - stst) + "ms,共搜索到 " + cc
            + "个词条！");
